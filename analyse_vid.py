...
def find_obstacle(mask, blue_coord, yellow_coord):
    ...
    if obstacles:
        ...
        for obs in obstacles:
            ...
            if rightmost[0] < blue_coord[0] or leftmost[0] > yellow_coord[0]:
                continue
        ...
        return calc_distances(coordinates, blue_coord, yellow_coord)
        
    # if there are no obstacles, return the coordinates given as input
    if yellow_coord[0] < blue_coord[0]:
        yellow_coord = YELLOW_MISSING_COORD
    elif blue_coord[0] > yellow_coord[0]:
        blue_coord = BLUE_MISSING_COORD
    return (blue_coord, yellow_coord)

def calc_distances(coordinates, blue_coord, yellow_coord):
...
BLUE_MIN = np.array([77, 58, 106])
BLUE_MAX = np.array([135, 255, 255])

YELLOW_MIN = np.array([20, 80, 125])
YELLOW_MAX = np.array([50, 215, 200])
...
RED_MIN = np.array([0, 189, 89])
RED_MAX = np.array([20 ,255,188])
